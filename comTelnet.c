...
struct pollfd pfd[MAX_USERS + 1+1];
...
int main(int argc, char **argv) {
	...
	strncpy(serial_dev, argv[2], 32);

	fd = serialOpen(serial_dev, 115200);
	if (fd < 0) {
	...
	pfd[MAX_USERS+1].fd = fd;
	pfd[MAX_USERS+1].events = POLLIN;
